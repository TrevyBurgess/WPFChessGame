...
namespace TrevyBurgess.Games.TrevyChess.ChessBoardLogic.Internal
{
    ...
    /// </summary>
    internal class PawnWhite : ChessPieceBase
    {
        internal PawnWhite(ChessBoard board, ChessPieceColor Color) : base(board, Color, ChessPieceType.Pawn) { }
        ...
        internal override bool CanMove(ChessPosition newPosition)
        {
            ...
            if (Board[newPosition.ColLoc, newPosition.RowLoc].PieceColor == ChessPieceColor.White)
                return false;
            ...
            else if (iEndCol == iStartCol + 1 || iEndCol == iStartCol - 1)
            {
                ...
                {
                    PawnBlack pawn = Board.GetChessPiece((ColPos)iEndCol, (RowPos)(iEndRow - 1)) as PawnBlack;
                    if (pawn == null)
                    {
        ...
        internal override List<ChessMove> PossibleMoveList()
        {
            ...
            // Capture to left
            if (iStartRow < 8 && iStartCol > 1 && Board[iStartCol - 1, iStartRow - 1].PieceType != ChessPieceType.EmptySquare)
            {
                var move = new ChessMove();
                move.OldRookPosition.ColLoc = MyPosition.ColLoc;
                move.OldRookPosition.RowLoc = MyPosition.RowLoc;
        ...
        internal override ChessMove DoMoveHousekeeping(ChessPosition oldPosition, ChessPosition newPosition)
        {
            ...
            if (base.theChessPiece.PieceColor == ChessPieceColor.White && oldPosition.RowLoc == newPosition.RowLoc - 2)
            {
                MovedTwoSpaces = true;
